...
class MyDataset(Dataset):
    def __init__(self, cfg, mode="train"):
        ...
        print(f"2A3: {len(df_2A3)}, DMS: {len(df_DMS)}")

        split = list(KFold(n_splits=fold_num, random_state=2024,
        ...
        if not cfg.task.oof:
            ...
            sample_frac = cfg.task.sample_frac
            shared_indices = df_2A3.sample(frac=sample_frac, random_state=42).index  # 生成共享索引
            df_2A3 = df_2A3.loc[shared_indices].reset_index(drop=True)
            df_DMS = df_DMS.loc[shared_indices].reset_index(drop=True)
            print(f"2A3: {len(df_2A3)}, DMS: {len(df_DMS)}")
